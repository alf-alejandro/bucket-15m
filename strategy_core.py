...
SLOT_STEP   = 900          # 15 minutos
TOP_LEVELS  = 15

SYMBOL      = os.environ.get("SYMBOL", "SOL").upper()
SLUG_PREFIX = "btc-updown-15m" if SYMBOL == "BTC" else "sol-updown-15m"
...
def build_market_info(gamma_m, clob_m) -> dict | None:
    ...
    return {
        ...
        "question":         clob_m.get("question", "SOL Up/Down 15min"),
...
def find_active_market(symbol: str) -> dict | None:
    """
    Busca el mercado UP/DOWN 15m activo para el simbolo dado (SOL, BTC, ETH).
    Estrategia robusta: genera todos los slots posibles alineados a multiplos
    de SLOT_STEP en la ultima hora. Funciona aunque el origen cambie.
    """
    ...
    now  = int(time.time())
    # Base alineada al multiplo de 900 mas cercano hacia abajo
